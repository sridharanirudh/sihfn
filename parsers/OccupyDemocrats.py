...
files = glob.glob('data/articles/OccupyDemocrats/*')
orientation = "left"


tweetFile = gzip.open('data/tweets/OccupyDemocrats.json.gz')
tweets = json.loads(tweetFile.read())

portal = "occupy-democrats"
veracity = "mostly false"

tweet_hash = {}
for tweet in tweets:
    tweet_hash[f'data/articles/OccupyDemocrats/{tweet["id_str"]}'] = tweet
...
for fname in files[-25:]:
    ...
    if div:
        ...
        if mainText:
            ...
            tree.write(f"/home/anirudh/ub/cse702/sihfn/data/parsed/OccupyDemocrats/{findex}.xml")

            x = etree.parse(f"/home/anirudh/ub/cse702/sihfn/data/parsed/OccupyDemocrats/{findex}.xml")
            f = open(f"/home/anirudh/ub/cse702/sihfn/data/parsed/OccupyDemocrats/{findex}.xml", "wb+")
